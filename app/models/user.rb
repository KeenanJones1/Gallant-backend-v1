class User < ApplicationRecord
 ...
 validates :middle_int, format: {:with => /\a-zA-Z/, message: "Only a single letter"}
 ...
 validates :email, format: {with: /^(|(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6})$/i, message: "Please use the johndoe@email.com format"}
